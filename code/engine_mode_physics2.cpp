...
UpdateAndRenderPhysics2(game_memory *Memory, game_input *Input)
{
    ...
    if(!ModePhysics2->IsInitialized)
    {
        ...
        ModePhysics2->ControlledEntityArray[0].EntityIndex = 0;
        ...
        ++ControllerIndex)
    {
        ...
        controlled_entity *ConEntity = ModePhysics2->ControlledEntityArray + ControllerIndex;
        ...
        if(ControllerIndex == 0)
        {
            ...
            if(WasPressed(Controller->ActionUp))
            {
                ...
                ConEntity->EntityIndex = Clamp(0, ConEntity->EntityIndex, ArrayCount(ModePhysics2->EntityArray) - 1);
            }
        ...
        ++EntityIndex)
    {
        ...
        for(u32 ControllerIndex = 0;
            ControllerIndex < ArrayCount(ModePhysics2->ControlledEntityArray);
            ++ControllerIndex)
        {
            controlled_entity *ConEntity = ModePhysics2->ControlledEntityArray + ControllerIndex;
            if(ControllerIndex == 0)
