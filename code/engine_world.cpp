...
IsCanonical(r32 ChunkDim, r32 TileRel)
{
    ...
    r32 Epsilon = 0.01f;
    b32 Result = ((TileRel >= -(0.5f * ChunkDim + Epsilon)) && (TileRel <= (0.5f * ChunkDim + Epsilon)));
...
{
    b32 Result =
    (IsCanonical(World->ChunkDimInMeters.x, Offset.x) && IsCanonical(World->ChunkDimInMeters.y, Offset.y) &&
...
{
    v3 dTile = {(r32)A->ChunkX - (r32)B->ChunkX, (r32)A->ChunkY - (r32)B->ChunkY,
