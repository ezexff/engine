internal sim_entity_hash *GetHashFromStorageIndex(sim_region *SimRegion, u32 StorageIndex)
{
    Assert(StorageIndex);
...
inline sim_entity *GetEntityByStorageIndex(sim_region *SimRegion, u32 StorageIndex)
{
...
internal sim_entity *AddEntity(game_state *GameState, sim_region *SimRegion, u32 StorageIndex, low_entity *Source,
                               v2 *SimP);
...
internal sim_entity *AddEntityRaw(game_state *GameState, sim_region *SimRegion, u32 StorageIndex, low_entity *Source)
{
    Assert(StorageIndex);
    sim_entity *Entity = 0;
...
internal b32 TestWall(r32 WallX, r32 RelX, r32 RelY, r32 PlayerDeltaX, r32 PlayerDeltaY, r32 *tMin, r32 MinY,
                         r32 MaxY)
{
    b32 Hit = false;
...
internal void MoveEntity(sim_region *SimRegion, sim_entity *Entity, r32 dt, move_spec *MoveSpec, v2 ddP)
{
    ...
    for(u32 Iteration = 0; Iteration < 4; ++Iteration)
    {
        ...
        if(IsSet(Entity, EntityFlag_Collides) && !IsSet(Entity, EntityFlag_Nonspatial))
        {
            ...
            for(u32 TestHighEntityIndex = 0; TestHighEntityIndex < SimRegion->EntityCount; ++TestHighEntityIndex)
            {
                ...
                if(Entity != TestEntity)
                {
                    if(IsSet(TestEntity, EntityFlag_Collides) && !IsSet(Entity, EntityFlag_Nonspatial))
                    {
                        ...
                        if(TestWall(MinCorner.x, Rel.x, Rel.y, PlayerDelta.x, PlayerDelta.y, &tMin, MinCorner.y,
                                    MaxCorner.y))
                        {
                            WallNormal = V2(-1, 0);
                        ...
                        if(TestWall(MaxCorner.x, Rel.x, Rel.y, PlayerDelta.x, PlayerDelta.y, &tMin, MinCorner.y,
                                    MaxCorner.y))
                        {
                            WallNormal = V2(1, 0);
                            HitEntity = TestEntity;
                        ...
                        if(TestWall(MinCorner.y, Rel.y, Rel.x, PlayerDelta.y, PlayerDelta.x, &tMin, MinCorner.x,
                                    MaxCorner.x))
                        {
                            WallNormal = V2(0, -1);
                            HitEntity = TestEntity;
                        ...
                        if(TestWall(MaxCorner.y, Rel.y, Rel.x, PlayerDelta.y, PlayerDelta.x, &tMin, MinCorner.x,
                                    MaxCorner.x))
                        {
                            WallNormal = V2(0, 1);
                            HitEntity = TestEntity;
