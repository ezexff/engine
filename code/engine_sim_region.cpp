        ...
        if(PlayerDeltaLength > 0.0f)
        {
            ...
            for(uint32 TestHighEntityIndex = 0; TestHighEntityIndex < SimRegion->EntityCount; ++TestHighEntityIndex)
            {
                ...
                if(CanCollide(GameState, Entity, TestEntity))
                {
                    ...
                    v3 VolumeDim = GetDim(Entity->Collision);
                    v3 VolumeP = GetCenter(Entity->Collision);

                    v3 TestVolumeDim = GetDim(TestEntity->Collision);
                    v3 TestVolumeP = GetCenter(TestEntity->Collision);

                    v3 MinkowskiDiameter = {TestVolumeDim.x + VolumeDim.x, TestVolumeDim.y + VolumeDim.y,
                                            TestVolumeDim.z + VolumeDim.z};

                    v3 MinCorner = -0.5f * MinkowskiDiameter;
                    v3 MaxCorner = 0.5f * MinkowskiDiameter;

                    v3 Rel = ((Entity->P + VolumeP) - (TestEntity->P + TestVolumeP));

                    // TODO(casey): Do we want an open inclusion at the MaxCorner?
                    if((Rel.z >= MinCorner.z) && (Rel.z < MaxCorner.z))
                    {
                        test_wall Walls[] = {
                            {MinCorner.x, Rel.x, Rel.y, PlayerDelta.x, PlayerDelta.y, MinCorner.y, MaxCorner.y,
                             V3(-1, 0, 0)},
                            {MaxCorner.x, Rel.x, Rel.y, PlayerDelta.x, PlayerDelta.y, MinCorner.y, MaxCorner.y,
                             V3(1, 0, 0)},
                            {MinCorner.y, Rel.y, Rel.x, PlayerDelta.y, PlayerDelta.x, MinCorner.x, MaxCorner.x,
                             V3(0, -1, 0)},
                            {MaxCorner.y, Rel.y, Rel.x, PlayerDelta.y, PlayerDelta.x, MinCorner.x, MaxCorner.x,
                             V3(0, 1, 0)},
                        };
